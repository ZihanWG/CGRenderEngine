...
{
    vec4 baseColor = vec4(uMaterialAlbedo.rgb, clamp(uMaterialParams1.y, 0.0, 1.0));
    if (uMaterialTextureFlags0.x > 0.5)
    {
        baseColor *= texture(uBaseColorTexture, fs_in.TexCoord);
    }

    return vec4(max(baseColor.rgb, vec3(0.0)), clamp(baseColor.a, 0.0, 1.0));
}
...
void main()
{
    ...
    vec4 materialBaseColor = SampleBaseColor();
    vec3 materialAlbedo = materialBaseColor.rgb;
    float materialOpacity = materialBaseColor.a;
    ...
    AlbedoColor = vec4(materialAlbedo, materialOpacity);
    NormalColor = vec4(normal * 0.5 + 0.5, 1.0);
    MaterialColor = vec4(materialMetallic, materialRoughness, ambientOcclusion, materialOpacity);
}
